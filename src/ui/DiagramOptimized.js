...
import './Diagram.css';
/**
 * Optimized color scheme with futuristic neon palette
 */
const NODE_COLORS = {
    module: { main: 0x00d9ff, glow: 0x3b82f6, label: 'Module' }, // Neon Cyan
    function: { main: 0x7c3aed, glow: 0xa855f7, label: 'Function' }, // Neon Violet
    class: { main: 0x06b6d4, glow: 0x22d3ee, label: 'Class' }, // Electric Cyan
    variable: { main: 0xec4899, glow: 0xf472b6, label: 'Variable' }, // Neon Pink
    loop: { main: 0xa855f7, glow: 0xc084fc, label: 'Loop' }, // Purple
    conditional: { main: 0xef4444, glow: 0xf87171, label: 'Conditional' }, // Red
    namespace: { main: 0x3b82f6, glow: 0x60a5fa, label: 'Namespace' }, // Blue
    interface: { main: 0x14b8a6, glow: 0x2dd4bf, label: 'Interface' }, // Teal
    struct: { main: 0xf59e0b, glow: 0xfbbf24, label: 'Struct' }, // Amber
    property: { main: 0x10b981, glow: 0x34d399, label: 'Property' }, // Emerald
    import: { main: 0x6b7280, glow: 0x9ca3af, label: 'Import' }, // Gray
};
const NODE_SIZES = {
    module: 1.8,
    class: 1.4,
...
const Diagram = ({ visualizationData, isVisualizing, onSelectNode, selectedNodeId, code = '' }) => {
    ...
    useEffect(() => {
        ...
        visualizationData.nodes.forEach((node) => {
            ...
            if (!geometry) {
                geometry = createNodeGeometry(nodeType, size, 12); // Reduced detail for performance
        ...
        visualizationData.edges.forEach((edge) => {
            ...
            switch (edge.type) {
                case 'call':
                    edgeColor = 0x7c3aed; // Violet
                    edgeWidth = 2;
                    break;
                case 'data':
                    edgeColor = 0x00d9ff; // Cyan
                    edgeWidth = 1.5;
                    break;
                case 'control':
                    edgeColor = 0x06b6d4; // Electric Cyan
                    edgeWidth = 1.5;
                    break;
            }
            let curve;
            ...
            const points = curve.getPoints(30); // Reduced points for performance
        ...
        // Add subtle grid
        const gridHelper = new THREE.GridHelper(30, 30, 0x00d9ff20, 0x00d9ff10);
